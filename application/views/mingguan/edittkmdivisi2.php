...
<div class="main-content">
  <section class="section">
    <div class="section-header">
      <h1>Edit Target Kerja Mingguan Direksi 2</h1>
          ...
          <div class="card">
            ...
            <div class="card-body">
              ...
              <!-- <form action="<?php echo base_url('mingguan/simpantkm2')?>" method="POST">
                ...
                <!-- ARRAY PERSENTASE -->
                <?php $persen = []; foreach ($persentase as $db ) {
                    $key = $db['project'];
                ...
                <ol class="umum">
                  ...
                  $waitinglist = $this->db->query("SELECT * FROM list_note a JOIN tb_user b ON a.username=b.id_user WHERE b.divisi='$divisinya' AND a.to_tkm = 'N'")->result_array();
                      ...
                      <div class="row">
                        <div class="col-sm-4">
                          <div class="form-group">
                            ...
                            <input type="text" class="form-control" id="project<?php echo $cp['no']; ?>" name="project<?php echo $cp['no']; ?>" value="<?php echo $cp['project']?>" onchange="project(<?=$cp['no']?>)" disabled>
                          </div>
                        </div>

                        <!-- <div class="col-sm-4">
                          <div class="form-group">
                            <label for="deskripsi<?php //echo $cp['no']; ?>">Keterangan :</label>
                            <input type="text" class="form-control" id="deskripsi<?php //echo $cp['no']; ?>" name="deskripsi<?php //echo $cp['no']; ?>" value="<?php //echo $cp['deskripsi']?>" onchange="ubah('deskripsi', <?php //echo $cp['no']?>)">
                          </div>
                        </div> -->
                        ...
                        <div class="col-sm-4">
                          <div class="form-group">
                            ...
                            <ol id="ul<?=$cp['no']?>">
                                <section id="li<?=$cp['no']?>">
                                    <?php $key=$cp['no']; foreach($urai[$key] as $xy):?>
                                        <?php if($xy['uraian']!=''):?>
                                        <li id="<?=$cp['no']?>**<?=$xy['id_uraian']?>"><?=$xy['uraian']?>
                                         <input type="date" name="targetselesai" class="form-control" value="<?= $xy['targetselesai'] ?>" title="Target Selesai" readonly>
                                            ...
                                            <a href="javascript:void(0)" title="Delete" class="text-danger mr-3" data-toggle="modal" data-target="#hapus" data-id_uraian="<?=$xy['id_uraian']?>" data-uraian="<?=$xy['uraian']?>" data-project="<?=$cp['project']?>" data-id_pekerjaan="<?=$cp['no'];?>"><i class="fas fa-trash"></i></a>
                                        </li>
                                        <?php endif?>
                                    <?php endforeach?>
                                </section>
                ...
                <div class="form-group">
                  <?php if ($this->session->userdata('ses_jabatan') != 'Staff') {
                    ?>
                  <a href="<?=base_url('mingguan/approve')?>/<?php echo $tkm['no']; ?>" class="btn btn-primary btn-block btn-round">Selesai dan Setujui</a>
            ...
            <form action="<?=base_url('mingguan/edituraian')?>" method="post">
              <div class="modal-body">
                ...
                <div class="form-group">
                  ...
                  <input type="date" class="form-control" id="targetmodal" name="targetmodal" value="" required>
